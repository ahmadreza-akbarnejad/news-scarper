...
class NewsScraper:
    ...
    def _extract_post_data(self, i, title, date, description, picture):
    ...
    def _get_post_data(self, i, post):

        title = post.find_element(By.CSS_SELECTOR, '.promo-title .link')
        date = post.find_element(By.CSS_SELECTOR, '.promo-timestamp')

        try:
            description = post.find_element(By.CSS_SELECTOR, '.promo-description')
        ...
        try:
            picture = post.find_element(By.CSS_SELECTOR, '.image')
        except Exception as e:

            self.logger.error(e)
            picture = None

        return self._extract_post_data(i, title, date, description, picture)

    def _get_posts_data(self):
        posts = self.browser.find_elements('css:.promo-wrapper')

        with ThreadPoolExecutor() as executor:
            posts_elements = executor.map(self._get_post_data, range(len(posts)), posts)

        return list(posts_elements)

    def _get_page_news(self):
        self.logger.info("Fetching Data...")
        data = self._get_posts_data()
